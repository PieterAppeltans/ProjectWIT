...
SpMat diagonalise(VectorXd& x)
{
  SpMat D(x.rows(),x.rows());
...
SpMat dRudu(VectorXd& u,VectorXd& v)
{
  ArrayXd u_a = u.array();
  ArrayXd v_a = v.array();
  ArrayXd res_a = (Vmu*Kmu)*((1+v_a/Kmv)*(Kmu+u_a).pow(2)).inverse();
  VectorXd res_vec = res_a.matrix();
  SpMat m = diagonalise(res_vec);
  return m;
}

SpMat dRudv(VectorXd& u,VectorXd& v)
{
  ArrayXd u_a = u.array();
  ArrayXd v_a = v.array();
...
SpMat dRvdu(VectorXd& u,VectorXd& v)
{
  ArrayXd u_a = u.array();
  ArrayXd v_a = v.array();
...
SpMat dRvdv(VectorXd& u,VectorXd& v)
{
  return rq*dRudv(u,v);
}
